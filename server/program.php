...
?>
  ...
  <menu class="menu-buttons">
    <header class="top-bar full"><?php echo htmlspecialchars($name); ?></header>
...
<header class="posts-top-bar miiverse-with-attributes-top-bar" data-miiverse-app-data="<?php echo $userData['mii_language'] ?>"  data-miiverse-search-key="<?php echo $program_id ?>"  data-miiverse-topic-tag="<?php if (strlen($name) > 50) { $name = substr($name, 0, 47) . "..."; echo htmlspecialchars($name); } else { echo htmlspecialchars($name); } ?>"><?php echo localize("vino.program.olv.viewer.header", ["%s" => htmlspecialchars($name)]); ?></header>
