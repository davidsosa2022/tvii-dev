...
?>
  ...
  <menu class="menu-buttons">
    <header class="top-bar full" data-miiverse-search-key="<?php echo $program_id ?>"  data-miiverse-topic-tag="<?php if (strlen($name) > 50) { $name = substr($name, 0, 47) . "..."; echo htmlspecialchars($name); } else { echo htmlspecialchars($name); } ?>"><?php echo htmlspecialchars($name); ?></header>
...
<div class="miiverse-posts program">
  <a no_touch class="loading_miiverse"></a>
  <button data-hover navi_target class="back_white_button miiverse-back accesskey-b" data-sound="SE_RETURN_TOUCH_OFF"></button>

<header class="posts-top-bar"><?php echo localize("vino.program.olv.viewer.header", ["%s" => htmlspecialchars($name)]); ?></header>
